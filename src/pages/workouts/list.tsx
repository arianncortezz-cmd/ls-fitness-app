...
export default function WorkoutsList() {
  ...
  const getStudentName = (studentId: string) =>
    students.find((s) => s.id === studentId)?.name ?? "Aluna não encontrada";
    ...
    <AppLayout>
      <div className="px-6 py-7 flex flex-col gap-6">
        ...
        <div className="flex items-start justify-between gap-4 animate-ls-slide-up">
          <div>
            <h1 className="text-[1.375rem] font-bold text-foreground tracking-[-0.02em]">Treinos</h1>
            <p className="text-sm text-muted-foreground mt-0.5">Gerencie todos os treinos das suas alunas.</p>
          </div>
          <button
        ...
        {workouts.length === 0 ? (
          <div className="bg-white rounded-2xl border border-border shadow-sm flex flex-col items-center gap-4 py-16 text-center animate-ls-slide-up">
            ...
            <div>
              <p className="text-sm font-semibold text-foreground">Nenhum treino criado ainda.</p>
              <p className="text-xs text-muted-foreground mt-1">Crie o primeiro treino para uma aluna.</p>
            </div>
            <button
              onClick={() => navigate("/workouts/new")}
            ...
            {workouts.map((workout, i) => (
              ...
              >
                ...
                <div>
                  ...
                  <p className="text-xs text-muted-foreground mt-1">{getStudentName(workout.studentId)}</p>
                </div>
